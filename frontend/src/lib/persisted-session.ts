...
export function loadPersistedSession(): Partial<PersistedSession> | null {
  ...
  if (!parsed || parsed.v !== SESSION_VERSION || !parsed.data) return null
  return parsed.data
}
...
export function savePersistedSession(data: PersistedSession): void {
  ...
  try {
    localStorage.setItem(SESSION_KEY, JSON.stringify({ v: SESSION_VERSION, data }))
  } catch {
    /* quota / private mode */
  }
}
...
export function loadRagMessages(): RagMessage[] {
  ...
  if (!parsed || parsed.v !== SESSION_VERSION || !Array.isArray(parsed.messages)) return []
...
export function saveRagMessages(messages: RagMessage[]): void {
  if (typeof window === 'undefined') return
  try {
    localStorage.setItem(RAG_KEY, JSON.stringify({ v: SESSION_VERSION, messages }))
  } catch {
    /* quota */
  }
}
